...
/**
 * 币种支持。TODO: 根据当前用户的locale，自动指定币种
 * 
 * @author jamesp
 */
public class Money implements Serializable, Cloneable, Comparable<Money>, Externalizable {
	...
	public Money(int amount, Currency currency) {
		this(
				BigDecimal.valueOf(amount).setScale(BankersRounding.getScaleForCurrency(currency),
						RoundingMode.HALF_EVEN), currency);
	}

	public Money(int amount, String currencyCode) {
		this(BigDecimal.valueOf(amount).setScale(
	...
	public Money add(Money other) {
		if (!other.getCurrency().equals(getCurrency())) {
			throw new UnsupportedOperationException(
					"No currency conversion service is registered, cannot add different currency " + "types together ("
							+ getCurrency().getCurrencyCode() + " " + other.getCurrency().getCurrencyCode() + ")");
		}
	...
	public Money subtract(Money other) {
		if (!other.getCurrency().equals(getCurrency())) {
			throw new UnsupportedOperationException(
					"No currency conversion service is registered, cannot subtract different currency " + "types ("
							+ getCurrency().getCurrencyCode() + ", " + other.getCurrency().getCurrencyCode() + ")");
		}
	...
	public Money divide(BigDecimal divisor, RoundingMode roundingMode) {
		return new Money(amount.divide(divisor, amount.scale(), roundingMode), currency,
				amount.scale() == 0 ? BankersRounding.getScaleForCurrency(currency) : amount.scale());
	}
	...
	/**
	 * Ensures predictable results by converting the double into a string then
	 * calling the BigDecimal string constructor.
	 * 
	 * @param amount
	 *            The amount
